...
public class DataUserConfiguration{
	...
	SecurityFilterChain filterChain(HttpSecurity http) throws Exception{
		...
		http.authorizeHttpRequests(authorize -> authorize
			...
			.requestMatchers("/login-error", "/registro","/", "/home","/login", "/logout", "/inicioSesion", "/public/**","/FormLogin").permitAll()
	//		.requestMatchers("/rest/encriptar/**").permitAll()
			// Todas las demás URLs de la Aplicación requieren autenticación
			// Asignar permisos a URLs por ROLES
	 		.requestMatchers("/app/producto/**").hasAnyAuthority("ROLE_ADMON")
			.requestMatchers("/app/usuarios/**").hasAnyAuthority("ROLE_ADMON")
			.requestMatchers("/app/perfiles/**").hasAnyAuthority("ROLE_ADMON")
			.requestMatchers("/app/tipos/**").hasAnyAuthority("ROLE_CLIENTE") 
